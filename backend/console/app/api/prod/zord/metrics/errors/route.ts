...
export async function GET(request: NextRequest) {
  return withDemoAnalytics((analytics) => {
    ...
    try {
      const data = analytics.getErrorIntelligenceMetrics(ctx.tenantId, ctx.timeRange)
      return withNoStore(demoJson(data))
    } catch (error) {
      return demoJson(
        { error: 'Failed to load error intelligence', detail: error instanceof Error ? error.message : 'unknown error' },
        { status: 500 },
      )
    }
  })
